...
def convert_stoi(str_list, event_dict):
    return [event_dict.get(s, 0) for s in str_list]


def load_data(data_dir, middle_dir="", log_file="", is_mapping=False):
    """
    ...
    abnormal_index = shuffle(np.arange(len(abnormal)), random_state=88)
    abnormal = abnormal[abnormal_index]

    train_abnormal = abnormal[:train_abnormal_size]
    test_abnormal = abnormal[train_abnormal_size:]

    assert len(train_abnormal) == train_abnormal_size

    print("Train normal size:", train.shape[0])
    print("Train abnormal size:", train_abnormal.shape[0])

    # combine train data
    x_train = np.vstack((train, train_abnormal))
    y_train = np.vstack((np.zeros(train.shape), np.ones(train_abnormal.shape)))
    ...
    if is_mapping:
        # eventids = np.unique(np.concatenate(x_train.squeeze(), axis=0))
        log_event = pd.read_csv(data_dir + log_file + "_templates.csv")
        eventids = log_event["EventId"].tolist()
        print("Total logkey(exclude 0:UNK)", len(eventids))
        ...
        for idx in range(x_train.shape[0]):
            x_train[idx] = convert_stoi(x_train[idx], event_dict)

    print("Test normal size:", test_normal.shape[0])
    print("Test abnormal size:", test_abnormal.shape[0])

    # combine test data
    x_test = np.vstack((test_normal, test_abnormal))
    y_test = np.vstack((np.zeros(test_normal.shape), np.ones(test_abnormal.shape)))
    ...
    if is_mapping:
        ...
        for idx in range(x_test.shape[0]):
            x_test[idx] = convert_stoi(x_test[idx], event_dict)
            num_unk_event += len(x_test[idx]) - sum(np.array(x_test[idx]) > 0)
        print(f"num_unk_event in test data: {num_unk_event}")
