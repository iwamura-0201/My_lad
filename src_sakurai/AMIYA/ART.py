...
UNK = 1
START = 2
data_dir = os.path.expanduser("/home/ssakurai/project/log/dataset/amiya/AtomicRT/")
output_dir = "/home/ssakurai/project/log/dataset/preprocessed/ART/"
log_file = "AtomicRedTeam.csv"
...
def prepre(data_path, label_path, csv_path):
    data = pd.read_excel(
        "/home/ssakurai/project/log/dataset/amiya/AtomicRT/取りまとめ後参考/command-and-control_20240927_timeline.xlsx"
    )
    ano = pd.read_excel(
        "/home/ssakurai/project/log/dataset/amiya/AtomicRT/取りまとめ後参考/AtomicRedTeam_Analyze.xlsx",
        sheet_name="解析結果",
    )

    # ラベルを取得（インデックスとして扱う準備）
    label = ano["Logs"].iloc[1:-3].tolist()

    # 型を確認してインデックスに変換
    label = [int(x) - 2 for x in label if pd.notna(x)]  # NaNを除外し、数値に変換

    data["Label"] = "-"
    data.loc[label, "Label"] = "anomaly"
...
if __name__ == "__main__":
    ...
    data_path = "/home/ssakurai/project/log/dataset/amiya/AtomicRT/取りまとめ後参考/command-and-control_20240927_timeline.xlsx"
    label_path = "/home/ssakurai/project/log/dataset/amiya/AtomicRT/取りまとめ後参考/AtomicRedTeam_Analyze.xlsx"
    csv_path = "/home/ssakurai/project/log/dataset/amiya/AtomicRT/AtomicRedTeam.csv"

    prepre(data_path, label_path, csv_path)
