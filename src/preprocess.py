...
def parse_log(input_dir, output_dir, logfile_name, parser_type):
    """
    ...
    if parser_type == "drain":
        ...
        parser.parse(f"{logfile_name}.csv")
    ...
    # リネーム
    os.rename(
        f"{output_dir}/{logfile_name}.csv_structured.csv",
        f"{output_dir}/{logfile_name}_structured.csv"
    )
    os.rename(
        f"{output_dir}/{logfile_name}.csv_templates.csv",
