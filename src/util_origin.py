...
from model.logbert.bert_no_time import BERTNoTime
from model.logbert.bert_pretrained import BERTWithPretrainedEmbedding
...
def suggest_network(cfg):
    if cfg.network.ver == "logbert":
        ...
        elif cfg.network.encoder.name == "bert_no_time":
            encoder = BERTNoTime(
                cfg.dataset.vocab.vocab_size,
                cfg.dataset.sample.seq_len,
                cfg.network.encoder.hidden_size,
        ...
        elif cfg.network.encoder.name == "bert_pretrained":
            encoder = BERTWithPretrainedEmbedding(
                pretrained_embed_path=cfg.network.encoder.pretrained_embed_path,
                eventid_mapping_path=cfg.network.encoder.eventid_mapping_path,
                max_len=cfg.dataset.sample.seq_len,
                hidden=cfg.network.encoder.hidden_size,
                n_layers=cfg.network.encoder.layer_num,
                attn_heads=cfg.network.encoder.attn_heads,
                dropout=cfg.network.encoder.dropout,
                freeze_pretrained=cfg.network.encoder.get('freeze_pretrained', True),
            )
...
def setup_device(cfg):
    if torch.cuda.is_available():
        ...
        try:
            ...
            if not found:
                print("Warning: No working CUDA device found. Falling back to CPU.")
                device = "cpu"
                cfg.default.device_id = device

        if device != "cpu" and not cfg.default.deterministic:
...
def cal_eval_matrix_per_project(test_normal_results, test_abnormal_results, 
                                 normal_projects, abnormal_projects,
                                 cfg, seq_range):
    """
    プロジェクトごと・閾値ごとに精度指標を計算する関数。
    
    Parameters
    ----------
    test_normal_results : dict
        正常テストの結果（各キーにtensorを含む）
    test_abnormal_results : dict  
        異常テストの結果（各キーにtensorを含む）
    normal_projects : np.ndarray
        各正常シーケンスのプロジェクト名
    abnormal_projects : np.ndarray
        各異常シーケンスのプロジェクト名
    cfg : OmegaConf
        設定
    seq_range : array-like
        評価する閾値のリスト
        
    Returns
    -------
    pd.DataFrame
        プロジェクトごと・閾値ごとの精度指標を含むDataFrame
    """
    import pandas as pd
    
    # 全プロジェクト名を取得
    all_projects = set(normal_projects.tolist()) | set(abnormal_projects.tolist())
    
    all_results = []
    
    for seq_threshold in seq_range:
        # 各シーケンスが異常と判定されたかどうかを計算
        # 正常データ
        normal_logkey_anomalies = (
            test_normal_results["undetected_tokens"] > test_normal_results["masked_tokens"] * seq_threshold
            if cfg.network.is_logkey
            else torch.zeros_like(test_normal_results["undetected_tokens"], dtype=torch.bool)
        )
        normal_time_anomalies = (
            test_normal_results["num_error"] > test_normal_results["masked_tokens"] * seq_threshold
            if cfg.network.is_time
            else torch.zeros_like(test_normal_results["num_error"], dtype=torch.bool)
        )
        normal_hypersphere = (
            test_normal_results["deepSVDD_label"] > 0
            if cfg.eval.hypersphere_loss_test
            else torch.zeros_like(test_normal_results["deepSVDD_label"], dtype=torch.bool)
        )
        normal_is_anomaly = (normal_logkey_anomalies | normal_time_anomalies | normal_hypersphere).cpu().numpy()
        
        # 異常データ
        abnormal_logkey_anomalies = (
            test_abnormal_results["undetected_tokens"] > test_abnormal_results["masked_tokens"] * seq_threshold
            if cfg.network.is_logkey
            else torch.zeros_like(test_abnormal_results["undetected_tokens"], dtype=torch.bool)
        )
        abnormal_time_anomalies = (
            test_abnormal_results["num_error"] > test_abnormal_results["masked_tokens"] * seq_threshold
            if cfg.network.is_time
            else torch.zeros_like(test_abnormal_results["num_error"], dtype=torch.bool)
        )
        abnormal_hypersphere = (
            test_abnormal_results["deepSVDD_label"] > 0
            if cfg.eval.hypersphere_loss_test
            else torch.zeros_like(test_abnormal_results["deepSVDD_label"], dtype=torch.bool)
        )
        abnormal_is_anomaly = (abnormal_logkey_anomalies | abnormal_time_anomalies | abnormal_hypersphere).cpu().numpy()
        
        # プロジェクトごとに集計
        for project in sorted(all_projects):
            # 正常データでこのプロジェクトに属するもの
            normal_mask = (normal_projects == project)
            FP = normal_is_anomaly[normal_mask].sum() if normal_mask.any() else 0
            TN = (~normal_is_anomaly[normal_mask]).sum() if normal_mask.any() else 0
            
            # 異常データでこのプロジェクトに属するもの
            abnormal_mask = (abnormal_projects == project)
            TP = abnormal_is_anomaly[abnormal_mask].sum() if abnormal_mask.any() else 0
            FN = (~abnormal_is_anomaly[abnormal_mask]).sum() if abnormal_mask.any() else 0
            
            # 精度指標の計算
            Precision = 100 * TP / (TP + FP) if (TP + FP) != 0 else 0
            Recall = 100 * TP / (TP + FN) if (TP + FN) != 0 else 0
            F1 = 2 * Precision * Recall / (Precision + Recall) if (Precision + Recall) != 0 else 0
            FPR = FP * 100 / (TN + FP) if (TN + FP) != 0 else 0
            TPR = TP * 100 / (FN + TP) if (FN + TP) != 0 else 0
            
            all_results.append({
                "threshold": round(seq_threshold, 3),
                "project": project,
                "FP": int(FP),
                "TP": int(TP),
                "TN": int(TN),
