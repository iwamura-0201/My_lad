...
def do_train(cfg):
    ...
    # Debug: vocab_sizeを確認
    print(f"DEBUG main.py: cfg.dataset.vocab.vocab_size = {cfg.dataset.vocab.vocab_size} (type: {type(cfg.dataset.vocab.vocab_size)})")
...
def do_test(weight_file_name, eval_batchsize, gpu):
    ...
    # プロジェクト別精度の計算と保存（per_project_evalがTrueの場合のみ）
    per_project_eval = getattr(cfg.eval, "per_project_eval", False)
    if per_project_eval and "normal_projects" in data_dict and "abnormal_projects" in data_dict:
        # pickleから結果を読み込み
        with open(output_dir_path + "/test_normal_results", "rb") as f:
            test_normal_results = pickle.load(f)
        with open(output_dir_path + "/test_abnormal_results", "rb") as f:
            test_abnormal_results = pickle.load(f)
        
        # プロジェクト情報を取得（drop_lastによる切り捨てを考慮）
        normal_projects = data_dict["normal_projects"]
        abnormal_projects = data_dict["abnormal_projects"]
        
        # 実際に評価されたサンプル数に合わせる
        normal_count = len(test_normal_results["masked_tokens"])
        abnormal_count = len(test_abnormal_results["masked_tokens"])
        normal_projects = normal_projects[:normal_count]
        abnormal_projects = abnormal_projects[:abnormal_count]
        
        # すべての閾値でプロジェクト別精度を計算
        project_df = cal_eval_matrix_per_project(
            test_normal_results, test_abnormal_results,
            normal_projects, abnormal_projects,
            cfg, seq_range=seq_range
        )
        project_df.to_csv(output_dir_path + "/test_result_per_project.csv", index=False)
        print(f"\n=== プロジェクト別精度 ===")
        print(f"結果を {output_dir_path}/test_result_per_project.csv に保存しました")
        print(f"閾値数: {len(seq_range)}, プロジェクト数: {len(project_df['project'].unique())}")
