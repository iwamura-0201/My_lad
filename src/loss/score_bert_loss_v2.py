...
- Weighted combination: final = score_err + β * event_err
"""
...
class ScoreBERTLoss(nn.Module):
    ...
    Combines two losses without normalization:
    ...
    Final loss = score_err + β * eventid_err
    
    Args:
        beta: Weight for eventid_err (default: 1.0)
    ...
    def forward(
        ...
        # ============================================
        # 3. Combined Loss (no normalization)
        # ============================================
        final_loss = score_err + self.beta * eventid_err
