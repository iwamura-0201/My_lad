...
- EventID embedding MSE loss as the primary training objective
- Score MSE loss computed for logging purposes only

Final loss = eventid_err
"""
...
class ScoreBERTLoss(nn.Module):
    ...
    Uses only EventID embedding loss for training:
    - eventid_err: MSE on masked EventID embedding positions
    - score_err: computed for logging only (not used in final loss)
    ...
    def forward(
        ...
        # ============================================
        # 3. Final Loss (eventid_err only)
        # ============================================
        final_loss = eventid_err

        # Return loss and components for logging
        # score_err is logged but not used in final_loss
        loss_dict = {
