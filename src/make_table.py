...
def make_table(encoder_names, dataset_names, train_ratios, seq_lens):
    ...
    if len(train_ratios) == 1:
        caption_parts.append(
            f"Train:Test Ratio: {train_ratios[0]}:{10-train_ratios[0]}"
        )
    else:
        param_axes.append("Train:Test")
        ...
        for encoder_name in encoder_names:
            for train_ratio in train_ratios:
                for seq_len in seq_lens:
                    file_path = f"./outputs/logbert/{encoder_name}/{dataset_name}_train{train_ratio}test{10-train_ratio}/seq_len_{seq_len}/r_seed_31/test_result.csv"
                    try:
                        # CSVファイルを読み込む
                        ...
                        if "Train:Test" in param_axes:
                            train_test = f"{train_ratio}:{10-train_ratio}"
                            latex_row.append(
                                train_test
                                if previous_row.get("Train:Test") != train_test
                                else ""
                            )
                            previous_row["Train:Test"] = train_test
    ...
    # 結果を保存
    with open("./results_table.tex", "w") as tex_file:
